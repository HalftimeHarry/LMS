					...
					{#each tableEntries as entry}
						...
						<tr class="border-b border-gray-800/50 hover:bg-white/[0.02]">
							...
							<td class="px-4 py-2 text-right">
								{#if deleteConfirmEntryId === e.id}
									<form method="POST" action="?/deleteEntry"
										use:enhance={() => async ({ update }) => { deleteConfirmEntryId = null; await update(); await invalidateAll(); }}
										class="inline-flex items-center gap-2"
									>
										<input type="hidden" name="id" value={e.id} />
										<button
											type="submit"
											class="rounded border border-red-500 bg-red-950/40 px-2 py-0.5 text-xs text-red-300 transition hover:bg-red-900/60"
										>
											Confirm
										</button>
										<button
											type="button"
											onclick={() => deleteConfirmEntryId = null}
											class="rounded border border-gray-700 bg-gray-900 px-2 py-0.5 text-xs text-gray-300 transition hover:bg-gray-800"
										>
											Cancel
										</button>
									</form>
								{:else}
									<button
										type="button"
										onclick={() => deleteConfirmEntryId = e.id}
										class="rounded border border-red-900 bg-red-950/30 px-2 py-0.5 text-xs text-red-400 transition hover:bg-red-950/60"
									>
										Delete
									</button>
								{/if}
